 ...
 * @param {number} fraction  Fraction correct, 0–1.
 * @returns {number}  Intensity in [0, 1]; 0 means "no glitter".
 */
export function glitterIntensity(fraction) {
 ...
 * @param {HTMLElement} host       Element to overlay + clip the effect to (give it position
 *                                 + overflow:hidden in CSS).
 * @param {number} intensity       0–1 (see {@link glitterIntensity}); scales count/size/energy.
 */
export function glitter(host, intensity) {
  ...
  /** @type {{x:number,y:number,vx:number,vy:number,size:number,color:string,rot:number,vrot:number,life:number,max:number,tw:number}[]} */
  const parts = [];
