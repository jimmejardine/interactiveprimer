...
/**
 * @typedef {"skipped" | "next" | "nearby" | "review"} UpNextKind
 ...
 * @param {(id: string) => number} args.starsOf  a concept's star rating (0 = unrated)
...
export function computeUpNext({ currentId, courseMembers, successors, starsOf, levelOf, titleOf }) {
  const isDone = (/** @type {string} */ id) => starsOf(id) > 0;
  /** @type {UpNextItem[]} */
  const items = [];
 ...
 * @param {(id: string) => number} levelOf
 * @param {(id: string) => string} titleOf
 * @returns {(a: string, b: string) => number}
 */
function byLevelProximity(curLevel, levelOf, titleOf) {
  return (a, b) => {
