...
/** @typedef {{ id: string, title?: string | null, course?: boolean }} StaleConcept */
/** @typedef {{ id: string, title: string, date: string }} StaleRow */
 ...
 *
 * @param {StaleConcept[]} concepts The concepts the build scanned (each carrying id/title/course).
 * @param {Map<string, number | Date>} mtimeById id → last-modified time (ms since epoch, or a Date).
 * @returns {StaleRow[]} `{ id, title, date }` rows, ascending by mtime then id.
 */
export function buildStaleRows(concepts, mtimeById) {
  const rows = [];
 ...
 * @returns {string}
 */
export function formatStaleRow(row) {
