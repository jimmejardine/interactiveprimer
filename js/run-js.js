 ...
 * @returns {string}
 */
export function formatArgs(values) {
 ...
 * @param {{ QuickJS: any, shouldInterruptAfterDeadline: any }} mod
 * @param {string} js  JavaScript source (already transpiled from TypeScript)
 * @param {{ timeoutMs?: number, memoryBytes?: number }} [opts]
 * @returns {{ ok: boolean, output: string[], error?: string }}
 */
export function runJs(mod, js, { timeoutMs = 1000, memoryBytes = 64 * 1024 * 1024 } = {}) {
  ...
  try {
    ...
    for (const method of ["log", "info", "warn", "error"]) {
      const fn = vm.newFunction(method, (/** @type {any[]} */ ...args) => {
