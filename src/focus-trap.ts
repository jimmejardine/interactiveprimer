...
/**
 * @returns the visible, focusable descendants of `container`, in tab order
 */
function focusable(container: ParentNode): HTMLElement[] {
  return ([...container.querySelectorAll(FOCUSABLE)] as HTMLElement[]).filter(
 ...
 * @param container the modal element to keep focus within
 * @param opts `initial` — the element to focus first
 *   (defaults to the first focusable, else the container itself)
 * @returns release — restores focus to the previously-focused element
 */
export function trapFocus(
  ...
  return () => {
    ...
    if (previouslyFocused && typeof previouslyFocused.focus === "function") {
      if (container.contains(document.activeElement as Node)) previouslyFocused.focus();
    }
  };
}
