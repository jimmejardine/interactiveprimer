// @ts-check
/**
 * src/boot.js — TEMPLATE for the single tag every concept page includes:
 *
 *   <script src="/js/boot.js"></script>
 *
 * The build (scripts/build.mjs) copies this to js/boot.js, replacing the one
 ...
 *
 * The whole framework + its deps now live in ONE content-hashed bundle under
 * /dist/bundle/ (built from js/ + node_modules by scripts/build.mjs); the static CSS
 * and fonts it needs are emitted to /dist/assets/. There is no /3rdparty/ anymore.
 *
...
(function boot() {
  ...
  // is still loaded on demand by js/theme.js.
  ...
  //    bundle self-boots (entry.js imports render.js) and inline scenes then resolve "primer"
  ...
  //    install (dist/precache.json) and keeps content fresh online (see sw.js). Deferred to `load`
  //    so it never competes with the first paint / the framework import above.
