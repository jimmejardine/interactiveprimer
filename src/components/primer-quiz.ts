 ...
 * registered with `registerQuiz(name, builder)` (see src/scenes.ts), referenced here by `name`. The
 ...
 *     template; `prompt`/`text`/`answer` may be functions of the drawn bindings (see src/quiz.ts):
 ...
 * having neither `options` nor `answer`; see extractConfig in src/quiz.ts): `num_questions` sets how
 ...
 * logic in src/quiz.ts (a variable template can be re-instantiated to produce many). Prompts/options
 * may contain LaTeX (wrapped in $…$), typeset with KaTeX.
 * @module
 */

import katex from "katex";
import { attachShared, awaitRegistration, katexHref } from "./shared.ts";
import { escapeHtml } from "../html-entities.ts";
import { generateQuiz, extractConfig } from "../quiz.ts";
import { getQuiz } from "../scenes.ts";
...
export class PrimerQuiz extends HTMLElement {
  /** The built bank, kept so "Try again" can re-draw a fresh quiz. */
  ...
  #ce: any = null;
  /** Tear-down for a pending "wait for quiz registration". */
  ...
  #resolve(root: ShadowRoot) {
    ...
    this.#cancelWait();
    let raw: Array<AuthoredQuestion | QuizConfig>;
    try {
  ...
  #render(root: ShadowRoot, quiz: GeneratedQuiz) {
    const item = (q: GeneratedQuestion, qi: number) => {
      ...
      void loadMathLive().then((ok) => {
        ...
        for (const input of polyInputs) {
          ...
          const onMoveOut = (e: Event) => {
            ...
            const dir = (e as any).detail?.direction;
            const target =
          ...
          const onMfEnter = (e: KeyboardEvent) => {
            if (e.key !== "Enter") return;
            e.preventDefault();
            e.stopPropagation();
  ...
  async #grade(root: ShadowRoot, quiz: GeneratedQuiz) {
    let score = 0;
    ...
    for (let qi = 0; qi < quiz.questions.length; qi++) {
      ...
      if (q.kind === "problem") {
        ...
        const probEl = questionEls[qi]?.querySelector("primer-geometry-problem") as any;
        if (probEl) {
        ...
        const progEl = questionEls[qi]?.querySelector("primer-program") as any;
        if (progEl) {
          correct = Boolean(await progEl.check?.());
          answered++; // an interactive exercise always counts as attempted on submit
        }
      } else if (q.kind === "text") {
        const input = root.querySelector(`input[name="q${qi}"]`) as HTMLInputElement | null;
        ...
        if (input) {
          ...
          const field = (row?.querySelector("math-field") ?? input) as HTMLElement;
        ...
        const chosen = root.querySelector(`input[name="q${qi}"]:checked`) as HTMLInputElement | null;
        ...
        for (const radio of root.querySelectorAll(`input[name="q${qi}"]`)) {
          const label = radio.closest(".option") as HTMLElement | null;
          if (!label) continue;
          if (Number((radio as HTMLInputElement).value) === q.correctIndex) {
    ...
    if (answered > 0) {
      ...
      if (gi > 0) {
        const card = root.querySelector(".quiz") as HTMLElement;
    ...
    const area = root.querySelector(".result-area") as HTMLElement | null;
    if (area) {
      area.innerHTML = `
        <div class="scorecard">
      ...
      const retry = area.querySelector(".retry") as HTMLButtonElement | null;
 ...
 */
function nextFocusStop(root: ShadowRoot, from: HTMLElement): HTMLElement | null {
  const questions = [...root.querySelectorAll(".q")];
  const currentQ = from.closest(".q");
  const nextQ = questions[questions.indexOf(currentQ as Element) + 1];
  if (nextQ) {
    return nextQ.querySelector('math-field, input.answer, input[type="radio"]') as HTMLElement | null;
  }
  return root.querySelector('button[type="submit"]') as HTMLElement | null;
}
 ...
 */
function prevFocusStop(root: ShadowRoot, from: HTMLElement): HTMLElement | null {
  const questions = [...root.querySelectorAll(".q")];
  const currentQ = from.closest(".q");
  const prevQ = questions[questions.indexOf(currentQ as Element) - 1];
  return prevQ
    ? (prevQ.querySelector('math-field, input.answer, input[type="radio"]') as HTMLElement | null)
    : null;
}
 ...
 */
function answerHtml(q: GeneratedTextQuestion): string {
