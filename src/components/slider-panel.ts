...
import { snapToAnchor } from "../chart-snap.ts";
import { escapeHtml } from "../html-entities.ts";

import type { SliderDef } from "../charts.ts";
 ...
 * @param initialValues  Seed for each control (else `value ?? min`).
 * @param onChange  Called (coalesced) with the full values.
 */
export function mountSliderPanel(
  host: HTMLElement,
  defs: SliderDef[],
  ...
  for (const r of host.querySelectorAll('input[type="range"]')) setFill(r as HTMLInputElement);
  ...
  const onInput = (e: Event) => {
    ...
    for (const other of host.querySelectorAll(`input[data-name="${name}"]`)) {
      if (other !== input) (other as HTMLInputElement).value = String(value);
    }
  ...
  const onClick = (e: Event) => {
    ...
    const btn = (target?.closest?.('[data-role="choice"]') ?? null) as HTMLElement | null;
    if (!btn) return;
...
/**
 * @returns The controls markup (one labelled slider + number box per def).
 */
function controlsHtml(defs: SliderDef[], values: Record<string, number>): string {
  return defs
    .map((p, i) => {
 ...
 */
function ticksHtml(p: SliderDef): string {
  const min = p.min ?? 0;
  const max = p.max ?? 0;
  const span = max - min;
 ...
 */
function updateTickDensity(host: HTMLElement) {
