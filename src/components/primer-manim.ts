...
import { attachShared, PLAY_ICON_SVG, BIG_PLAY_CSS } from "./shared.ts";
import { getManimScene } from "../scenes.ts";
import { makeStrings } from "../scene-strings.ts";
import { speak, cancelSpeech, pauseSpeech, resumeSpeech } from "../speech.ts";
...
export class PrimerManim extends HTMLElement {
  #state: "idle" | "playing" | "paused" | "done" = "idle";
  /** The active manim Scene (captured when the builder constructs it). */
  #scene: any = null;
  ...
  #onClick(root: ShadowRoot, btn: HTMLButtonElement) {
  ...
  async #start(root: ShadowRoot, btn: HTMLButtonElement) {
 ...
 */
function face(btn: HTMLButtonElement, icon: string, label: string) {
  btn.innerHTML = icon;
