...
import { attachShared, awaitRegistration, PLAY_ICON_SVG, BIG_PLAY_CSS } from "./shared.ts";
import { getGeometryScene } from "../scenes.ts";
import type { GeometryEntry } from "../scenes.ts";
...
import { getSliderGroup, subscribeSliders } from "../charts.ts";
import { clampStep, createStepCollector, applyStepVisibility } from "../geometry.ts";
import type { Waypoint } from "../geometry.ts";
...
export class PrimerGeometry extends HTMLElement {
  /** The active JSXGraph board. */
  #board: any = null;
  /** JSXGraph namespace (kept for freeBoard). */
  #jsx: any = null;
  #steps: Waypoint[] = [];
  /** Revealed-step count (0…stepCount). */
  #current: number = 0;
  /** Fade duration for a reveal, ms. */
  #stepMs: number = 450;
  #onTheme: (() => void) | null = null;
  #stopWaiting: (() => void) | null = null;
  /** Unsubscribe from a slider group. */
  #unsubscribe: (() => void) | null = null;
  /** Monotonic build id (abort a build superseded during its async await). */
  #buildGen: number = 0;
  /** Play-mode timer handle. */
  #playTimer: number = 0;
  /** Mini boards rendered in the expanded view (freed on collapse). */
  #miniBoards: any[] = [];
  #expanded: boolean = false;
  /** Whether the initial step has been chosen (so rebuilds keep the student's place). */
  #started: boolean = false;
  /** Per-run seed for the toolkit `rng` (bumped by Refresh; stable otherwise). */
  #seed: number | null = null;
  /** Whether this scene opted into random (shows the Refresh button). */
  #random: boolean = false;
  /** "Finished-frame-first" mode (the default for any multi-step scene): the figure
  ...
  connectedCallback() {
    ...
    this.#bigPlay = root.querySelector(".big-play") as HTMLElement;
  ...
  async #build(root: ShadowRoot) {
    ...
    try {
      ...
      this.#stepMs = Number.isFinite(entry.opts.stepMs) ? (entry.opts.stepMs as number) : 450;
      ...
      const heading = root.querySelector(".geo-title") as HTMLElement;
      ...
      if (!this.#started) {
        ...
        const opening = Number.isFinite(start) && (start as number) > 0
          ? (start as number)
   ...
   * expanded mini board.
   * @param name  The scene name (scopes the localized strings).
   */
  #runBuilder(host: HTMLElement, JXG: Record<string, any>, entry: GeometryEntry, name: string) {
    const colors = themeColors();
    const { boundingbox = [-5, 5, 5, -5], keepAspect = true } = entry.opts;
    let board = null;
    const wrapped = wrapBoard(JXG, colors, (b: any) => {
      board = b;
    });
    ...
    const rng = makeRng((this.#seed ?? 0) as number);
  ...
  #renderBar() {
    ...
    (root.querySelector(".refresh") as HTMLElement).hidden = !this.#random;
    const count = root.querySelector(".count") as HTMLElement;
    count.textContent = `${this.#current} / ${n}`;
    const caption = root.querySelector(".caption") as HTMLElement;
    caption.textContent = this.#current > 0 ? `${this.#current}. ${this.#steps[this.#current - 1]?.caption ?? ""}` : "";
    (root.querySelector(".rewind") as HTMLButtonElement).disabled = this.#current <= 0;
    (root.querySelector(".prev") as HTMLButtonElement).disabled = this.#current <= 0;
    (root.querySelector(".next") as HTMLButtonElement).disabled = this.#current >= n;
    (root.querySelector(".forward") as HTMLButtonElement).disabled = this.#current >= n;
  ...
  async #expand() {
    ...
    if (!entry) return;
    const root = this.#root;
    const expandedEl = root.querySelector(".expanded") as HTMLElement;
    const stage = root.querySelector(".stage") as HTMLElement;
    this.#stopPlay();
    try {
      const mod = await import("jsxgraph");
  ...
  #setExpandLabel(expanded: boolean) {
  ...
  #awaitRegistration(root: ShadowRoot, stage: HTMLElement, name: string) {
