...
import { attachShared, slug } from "./shared.ts";
...
export class PrimerConcept extends HTMLElement {
  #onQuizGraded: ((e: Event) => void) | null = null;
  #onCourse: (() => void) | null = null;
  ...
  connectedCallback() {
    ...
    const starEls = [...root.querySelectorAll(".star")] as HTMLButtonElement[];
    const statusEl = root.querySelector(".conf-status") as HTMLElement;
    ...
    const attn = root.querySelector(".attn") as HTMLButtonElement;
    const markDone = () => {
      attn.disabled = true;
    ...
    attn.addEventListener("click", () => {
      ...
      try {
        (window as any).goatcounter?.count?.(attentionEvent(id, title));
      } catch {
    ...
    if (meta?.course) {
      const wrap = root.querySelector(".course-focus") as HTMLElement;
      const btn = root.querySelector(".focus-course") as HTMLButtonElement;
      wrap.hidden = false;
      const refreshBtn = () => {
      ...
      const dlBtn = root.querySelector(".download-course") as HTMLButtonElement;
   ...
   * still shows a level.
   */
  async #showImplicitLevel(root: ShadowRoot, id: string) {
   ...
   */
  async #wireDownload(btn: HTMLButtonElement, id: string) {
    let offline;
    try {
      offline = await import("../offline.ts");
    } catch {
 ...
 */
function paint(stars: HTMLButtonElement[], upto: number) {
 ...
 */
function reflectRating(stars: HTMLButtonElement[], statusEl: HTMLElement, rating: number) {
  for (const s of stars) {
