...
import { get3dChart } from "../scenes.ts";
import { themeColors } from "../theme.ts";
import { t } from "../i18n.ts";
import { getSliderGroup, subscribeSliders } from "../charts.ts";
import { adoptJsxCss, disposeBoard, resolveJXG } from "./jsxgraph-board.ts";
import { reportError } from "../report-error.ts";

import type { Chart3dOptions } from "../scenes.ts";
...
export class PrimerChart3d extends HTMLElement {
  ...
  /** Tear-down for a pending "wait for registration". */
  #stopWaiting: (() => void) | null = null;
  /** Unsubscribe from the shared slider group. */
  ...
  #makeView(
    ...
    const label = (pt: number[], text: string) =>
