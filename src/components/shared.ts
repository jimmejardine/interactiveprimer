...
export const sharedSheet: CSSStyleSheet = new CSSStyleSheet();
sharedSheet.replaceSync(COMPONENT_CSS);

/**
 * Attach an open shadow root that has already adopted the shared stylesheet.
 */
export function attachShared(host: HTMLElement): ShadowRoot {
 ...
 */
export function katexHref(): string {
  return (
    (document.querySelector('link[rel="stylesheet"][href*="katex"]') as HTMLLinkElement | null)?.href ?? ""
  );
}
 ...
 * @param eventName  e.g. "primer:chart-registered"
 * @param name  the scene name to match on `detail.name`
 * @returns cancel
 */
export function awaitRegistration(
  eventName: string,
  name: string,
  { onReady, onTimeout, timeoutMs = REGISTRATION_TIMEOUT_MS }: { onReady: () => void; onTimeout?: () => void; timeoutMs?: number },
): () => void {
  let done = false;
  const onReg = (e: Event) => {
    if ((e as CustomEvent).detail?.name !== name) return;
    cancel();
    onReady();
  };
