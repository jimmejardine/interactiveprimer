...
import { getChart } from "../scenes.ts";
import { themeColors } from "../theme.ts";
import { t } from "../i18n.ts";
import { SLIDER_PANEL_CSS, mountSliderPanel } from "./slider-panel.ts";
import { groupForChart, getSliderGroup, subscribeSliders, setSliderValues, getChartMeta, resolveLegend } from "../charts.ts";
import { adoptJsxCss, disposeBoard, wrapBoard, resolveJXG } from "./jsxgraph-board.ts";
import { reportError } from "../report-error.ts";

export interface ChartParam {
  /** Key passed to the builder's update() (e.g. "A"). */
  name: string;
  /** Control label shown to the learner (defaults to `name`). */
  label?: string;
  min: number;
  max: number;
  /** Slider/number step (default 0.1). */
  step?: number;
...
export class PrimerChart extends HTMLElement {
  /** Inline (legacy) controls; empty when the chart has none / uses a group. */
  #params: ChartParam[] = [];
  /** Current values for the legacy inline-params path. */
  #values: Record<string, number> = {};
  /** The builder's re-plot fn. */
  #update: ((params: Record<string, number>) => void) | null = null;
  /** The active JSXGraph board (captured for disposal + theme rebuild). */
  #board: any = null;
  /** The JSXGraph namespace (`JXG.JSXGraph`), kept so #dispose can freeBoard. */
  #jsx: any = null;
  #onTheme: (() => void) | null = null;
  /** Tear-down for a pending "wait for chart registration". */
  #stopWaiting: (() => void) | null = null;
  /** Mounted slider panel (inline / legacy charts only). */
  #panel: { destroy: () => void } | null = null;
  /** Unsubscribe from a shared slider group. */
  #unsubscribe: (() => void) | null = null;
  /** Monotonic build id: a build superseded during its async await aborts. */
  #buildGen: number = 0;
   ...
   */
  #mountTitleAndControls(root: ShadowRoot, name: string) {
    ...
    const heading = root.querySelector(".chart-title") as HTMLElement;
   ...
   * chart has no legend.
   */
  #renderLegend(root: ShadowRoot, name: string) {
    const box = root.querySelector(".legend") as HTMLElement;
   ...
   * the created board on this element (for disposal + theme rebuild). See src/components/jsxgraph-board.ts.
   */
  #wrapJXG(JXG: Record<string, any>): Record<string, any> {
