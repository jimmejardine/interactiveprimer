...
let jsxCssPromise: Promise<CSSStyleSheet | null> | null = null;
 ...
 * @param mod  the awaited `import("jsxgraph")` module
 */
export function resolveJXG(mod: any): Record<string, any> {
 ...
 */
export function loadJsxCss(): Promise<CSSStyleSheet | null> {
  if (!jsxCssPromise) {
 ...
 * @param stillConnected  Guard so we don't adopt into a disconnected element.
 */
export function adoptJsxCss(root: ShadowRoot, stillConnected: () => boolean) {
 ...
 * @param JSXGraph  the `JXG.JSXGraph` namespace (has `freeBoard`)
 * @param board  the board to free (may be null)
 */
export function disposeBoard(JSXGraph: any, board: any) {
  if (!board) return;
  try {
...
export function wrapBoard(
  ...
  colors: ThemeColors,
  onBoard: (board: any, JSXGraph: any) => void,
  ...
  wrappedJSXGraph.initBoard = (box: any, attributes?: any) => {
