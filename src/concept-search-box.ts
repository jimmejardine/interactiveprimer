...
import { searchConcepts } from "./concept-search.ts";
...
export function mountSearchBox(
  host: HTMLElement,
  { items, onSelect, placement = "inline", placeholder = t("search.concepts") }: { items: { id: string, title: string, course?: boolean }[], onSelect: (id: string) => void, placement?: "inline" | "overlay" | "fixed", placeholder?: string },
): { destroy: () => void } {
  ...
  const input = search.querySelector(".cg-search-input") as HTMLInputElement;
  const list = search.querySelector(".cg-results") as HTMLElement;

  let results: { id: string, title: string, course?: boolean }[] = [];
  let activeIdx = -1;
  ...
  /** Move the active row (wrapping); -1 clears it. */
  const setActive = (i: number) => {
    ...
    if (activeIdx >= 0) {
      const el = els[activeIdx] as HTMLElement;
  ...
  const onListDown = (e: Event) => {
    const li = (e.target as Element).closest?.(".cg-result");
    if (!li) return;
    e.preventDefault();
 ...
 */
export function conceptSearchItems(
  byId: Map<string, { id: string, title?: string, titles?: Record<string, string>, course?: boolean }>,
  locale: string,
): { id: string, title: string, course: boolean }[] {
 ...
 */
export function mountConceptSearch(
  host: HTMLElement,
  { byId, locale, onSelect, placement, placeholder }: { byId: Map<string, any>, locale: string, onSelect: (id: string) => void, placement?: "inline" | "overlay" | "fixed", placeholder?: string },
): { destroy: () => void } {
 ...
 */
export function mountCourseSearch(
  host: HTMLElement,
  { byId, locale, onSelect, placement }: { byId: Map<string, any>, locale: string, onSelect: (id: string) => void, placement?: "inline" | "overlay" | "fixed" },
): { destroy: () => void } {
