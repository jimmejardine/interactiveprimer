/* sw.js — the Interactive Primer service worker (Phase 2 offline mode).
 *
 * Registered by js/boot.js (concept pages) and the standalone pages. Scope: "/". It gives the site
 * three offline capabilities, layered on the Phase-1 content-hashed build:
 *
 ...
 *   3. COURSE DOWNLOADS live in their own caches ("primer-course-<id>", written by js/offline.js). This
 *      SW only READS them on fetch (serve-then-revalidate); the client owns their lifecycle.
 *
 * No build step touches this file's cache NAMES beyond the shell version stamped into precache.json.
 */
...
const COURSE_PREFIX = "primer-course-"; // per-course caches owned by js/offline.js
...
const isGraph = (url) => url.pathname === "/dist/graph.json";
...
async function courseContent(request) {
