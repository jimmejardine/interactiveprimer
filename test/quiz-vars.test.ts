...
import {
  ...
  checkAnswer,
  formatValue,
} from "../src/quiz-vars.ts";

/** Deterministic LCG (matches test/quiz.test.ts). */
...
test("instantiate: deterministic, in range, reals 3dp, choice from list", () => {
  ...
  assert.equal(r1.b, Math.round((r1.b as number) * 1000) / 1000); // 3dp
  assert.ok([7, 8, 9].includes(r1.c as number));
});
...
test("instantiate: underscores in a string choice value become spaces", () => {
  ...
  const drawn = (drawBindings(vars, "", () => 0).task as string);
