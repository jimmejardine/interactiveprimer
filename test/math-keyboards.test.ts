...
import { getMathKeyboard } from "../src/math-keyboards.ts";
...
test("getMathKeyboard returns a layout with rows for a known name", () => {
  ...
  assert.ok(Array.isArray((kb as any).rows));
  assert.ok((kb as any).rows.length > 0);
});
...
test("geometry-angles has a degree key and Greek unknowns; geometry-lengths has neither", () => {
  const ang = getMathKeyboard("geometry-angles") as any;
  assert.ok(ang, "geometry-angles keyboard should exist");
  const angInserts = ang.rows.flat().map((c: any) => c.insert).filter(Boolean);
  ...
  const len = getMathKeyboard("geometry-lengths") as any;
  assert.ok(len, "geometry-lengths keyboard should exist");
  const lenCaps = len.rows.flat();
  const lenInserts = lenCaps.map((c: any) => c.insert).filter(Boolean);
  const lenLabels = lenCaps.map((c: any) => c.label);
