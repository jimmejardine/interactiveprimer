/**
 * Tests for the small pure helpers extracted during the dedup pass: escapeHtml (src/html-entities.ts)
 * and clamp (src/svg-util.ts). These protect the extractions that replaced ~a dozen copy-pasted copies.
 * (mk() needs a DOM and importUrl() needs a live URL, so they aren't unit-testable here.)
 */
import test from "node:test";
import assert from "node:assert/strict";
import { escapeHtml } from "../src/html-entities.ts";
import { clamp } from "../src/svg-util.ts";
...
test("escapeHtml leaves plain text untouched and handles edge cases", () => {
  ...
  assert.equal(escapeHtml(5 as any), "5");
});
