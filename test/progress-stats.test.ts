...
function fixture() {
  const byId = new Map([
    ...
    ["f", { prerequisites: [] as string[] }], // no prereqs → ready
  ]);
...
test("reviews queue is sorted most-overdue first", () => {
  const byId = new Map([
    ["x", { prerequisites: [] as string[] }],
    ["y", { prerequisites: [] as string[] }],
  ]);
  const entriesById = new Map([
...
test("a graduated 10/10 is never review-due even when stale; a same-day 10/10 still is", () => {
  const byId = new Map([
    ["g", { prerequisites: [] as string[] }],
    ["h", { prerequisites: [] as string[] }],
  ]);
  const entriesById = new Map([
...
test("pickNextConcept: ready-first (course order), then learning, then null", () => {
  ...
  const members = ["a", "b", "c"];
  const starsFrom = (s: Record<string, number>) => (id: string) => s[id] ?? 0;
