...
import { indexConcepts, neighborhood } from "../src/graph.ts";

const idx = (list: Array<{ id: string, prerequisites?: string[] }>) =>
  indexConcepts(list.map((c) => ({ title: c.id, prerequisites: [], ...c })));

/** Set of "a|b" edge keys for order-independent comparison. */
const edgeKeys = (edges: { a: string, b: string }[]) => new Set(edges.map((e) => `${e.a}|${e.b}`));
