...
import { computeRange, resolveLineStyle, resolveLegend, createSliderBroker } from "../src/charts.ts";
...
test("resolveLineStyle: a function receives live colours and the index", () => {
  const line = (colors: { line: string }, i: number) => ({ strokeColor: colors.line, dash: i });
...
test("resolveLegend: a function line style is evaluated per index", () => {
  const line = (colors: { cat: string[] }, i: number) => ({ strokeColor: colors.cat[i], dash: i === 0 ? 0 : 2 });
